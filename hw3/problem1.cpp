...
PetscErrorCode OrthogCheck(PetscInt n, Vec **q);

int main(int argc, char *argv[]){

   /*  setting up needed data types*/

   PetscInt nVec = 5 , nElem = 10; 
   ...
   PetscCall(GramSchmidt(nVec, v, &u_ptr));
   ...
   for (PetscInt i = 0; i < n; i++){
      for(PetscInt j = i + 1; j < n; j++){
         ...
         if(dot != 0 ){
            ...
            PetscCall(PetscPrintf(PETSC_COMM_WORLD,"Vectors q%d and q%d are not orthogonal\n", i, j));
            PetscCall(VecView((*q)[i],PETSC_VIEWER_STDOUT_WORLD));
            PetscCall(VecView((*q)[j],PETSC_VIEWER_STDOUT_WORLD));
         }
      }
   }
