...
void plot_maker(int xpts_global,int ypts_global, std::vector<uint32_t> &output_mesh ){
    ...
    const int non_root_ypts = ypts_global/worldSize; // the number of points all non-root cores will calculate
    const int remainder = ypts_global%worldSize; // The left over points from floor division
    const int root_ypts = non_root_ypts + remainder;// Non-root points plus the remainder  

    int ypts_loc;     // Number of x-direction points THIS core will calculate
    double ymin_loc;    // The "starting" x-point for THIS core
    if(rank == 0 ){
        ypts_loc = root_ypts;
        ymin_loc = (double) ymin_global ;
    }
    else{
        ypts_loc = non_root_ypts;
        ymin_loc = (double)ymin_global + yinc*(ypts_loc*rank + remainder);
    }
    ...
    const int mesh_size_loc = ypts_loc*xpts_global;
    ...
    */
    ...
    holder.reserve(ypts_loc);

    double cur_y = ymin_loc;    //current y position
    double cur_x = -2;   //current x position 
...
*/
    for(int iy = 0; iy < ypts_loc; ++iy){

        for(int ix = 0; ix < xpts_global; ++ix){
        ...
        holder.clear();
        holder.reserve(ypts_loc);
        c = std::complex<double>(xmin_global, c.imag()+yinc);
    }

    //populate output mesh with this row
    mesh_unifier(output_mesh, local_mesh);
...
void export_mesh(std::vector<uint32_t> &mesh){
    ...
    std::sprintf(fname, "res-%ld__Cores-%d__Rank-%d__Mandelbrot_set.txt", mesh.size(), worldSize, rank );
